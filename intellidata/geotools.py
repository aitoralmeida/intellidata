...
def obtain_shp_file(data, zipcode, identifier, key_field, data_hash, algorithm):
    """Generate shapefile with the same format as generate_zipcodes_map. Return the
    URL"""

    shp_file_without_extension = 'data/geo/%s_%s_%s_%s_%s' % (zipcode, identifier, key_field, algorithm, data_hash)
    ...
    if len(all_key_values) <= 1:
        ranked_multiplier = 256.0
    else:
        ranked_multiplier = 256.0 / (len(all_key_values) - 1)
    ...
    for zcode, zdata in data.iteritems():
        cur_key_value = zdata[key_field.lower()]
        if algorithm == Algorithms.LINEAR:
            position = cur_key_value / steps
        elif algorithm == Algorithms.LOGARITHMIC:
            if cur_key_value == 0.0:
                position = 0
            else:
                position = math.log10(cur_key_value) * max_logarithm
        else:
            if algorithm != Algorithms.RANKED:
                print "Unknown algorithm: %s. Using RANKED instead." % algorithm
            position = ranked_multiplier * all_key_values.index(cur_key_value)
